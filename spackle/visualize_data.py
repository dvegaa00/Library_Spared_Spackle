...
#TODO: eliminar visualizacion de medianas (dejar solo transformer)
def plot_pred_image(adata: ad.AnnData, n_genes: int = 3, slide = "", save_path="."):
    """
    ...
    adata.var[f'{name_model}_pcc_detailed'] = detailed_metrics['detailed_PCC-Gene']
    adata.var[f'{name_model}_mse_detailed'] = detailed_metrics['detailed_mse_gene']
    ...
    selected_genes = []
    n_top = adata.var.nlargest(gene_len, columns=f'{name_model}_pcc_detailed').index.to_list()
    n_bottom = adata.var.nsmallest(gene_len, columns=f'{name_model}_mse_detailed').index.to_list()
    ...
    def log_genes_for_slide(save_path, top_bottom, genes, slide_adata, gt_layer, trans_pred_layer, random_mask_layer, input_mask_layer):
        """
        ...
        # Declare figure
        fig, ax = plt.subplots(nrows=len(genes), ncols=4, layout='constrained')
        fig.set_size_inches(24, 6 * len(genes))
        ...
        for g in genes:         
            ...
            gene_min = min([gene_min_pred_1, gene_min_gt, gene_min_mask])
            gene_max = max([gene_max_pred_1, gene_max_gt, gene_max_mask])

            # Define color normalization
            norm = matplotlib.colors.Normalize(vmin=gene_min, vmax=gene_max)
            # Set PCC
            pcc_trans = str(round(slide_adata.var[f"{name_model}_pcc_detailed"][g], 3))
            # Set MSE
            mse_trans = str(round(slide_adata.var[f"{name_model}_mse_detailed"][g], 3))
            ...
            sq.pl.spatial_scatter(slide_adata, color=[g], layer=gt_layer, ax=ax[row,2], cmap='jet', norm=norm, colorbar=True, title="")
            
            # Set metrics as titles 
            ...
            ax[row,3].plot(ceros_gt_trans, ceros_gt_trans, color="black", linestyle="-", label="Ground Truth")
            ax[row,3].plot(ceros_gt_trans, ceros_trans_pred, color="blue", marker="o",  markersize=3, linestyle="None", label=f"SpaCKLE\nPCC = {pcc_trans} & MSE = {mse_trans}")
            ax[row,3].legend(markerfirst=3, framealpha=0.4, loc="lower right")
        ...
        # Set PCC
        pcc_trans = str(round(slide_adata.var[f"{name_model}_pcc_detailed"][genes[0]], 3))
        # Set MSE
        mse_trans = str(round(slide_adata.var[f"{name_model}_mse_detailed"][genes[0]], 3))
        # Set titles
        ax[0, 0].set_title('Masked Map', fontsize='xx-large')
        ax[0, 1].set_title(f'Prediction SpaCKLE\nPCC = {pcc_trans} & MSE = {mse_trans}', fontsize='xx-large')
        ax[0, 2].set_title('Ground Truth', fontsize='xx-large')
        ax[0, 3].set_title('Pred vs Trues', fontsize='xx-large')
        
        # Save plot 
        fig.savefig(os.path.join(save_path, f"{top_bottom}.jpg"))
        breakpoint()
        
    top_bottom = ["Top Genes", "Bottom Genes"]
    for i, gene in enumerate(selected_genes):
        log_genes_for_slide(name_model=name_model, save_path=save_path, top_bottom=top_bottom[i],genes=gene, slide_adata=slide_adata, gt_layer=gt_layer, trans_pred_layer=trans_pred_layer, random_mask_layer=random_mask_layer, input_mask_layer=input_mask_layer)
