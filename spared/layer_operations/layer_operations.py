...
# TODO: Add reference to the gtf_tools website in the documentation
# TODO: Add reference to TPM normalization paper
def tpm_normalization(organism: str, adata: ad.AnnData, from_layer: str, to_layer: str) -> ad.AnnData:
    ...
    This function uses the public implementation of gtf_tools.py to extract gene lengths from the gtf file.
...
def log1p_transformation(adata: ad.AnnData, from_layer: str, to_layer: str) -> ad.AnnData:
    ...
    Performs logarithmic transformation over the ``adata.layers[from_layer]`` matrix. Simple wrapper of scanpy's ``sc.pp.log1p()``
...
# FIXME: CHange the combat implementation to be updated (look at the pycombat github)
# TODO: Add other options of batch correction 
# TODO: Add reference to the ComBat paper
def combat_transformation(adata: ad.AnnData, batch_key: str, from_layer: str, to_layer: str) -> ad.AnnData:
    ...
    Args:
        adata (ad.AnnData): The AnnData object to transform. Must have logarithmically transformed data in ``adata.layers[from_layer]`` (ComBat assumes approximately normal distribution of gene expression).
...
# TODO: Put reference to SEPAL's use of deltas?
def get_deltas(adata: ad.AnnData, from_layer: str, to_layer: str) -> ad.AnnData:
    ...
    Args:
        adata (ad.AnnData): The AnnData object to update. Must have expression values in ``adata.layers[from_layer]`` and ``'train'`` values in the ``adata.obs['split']`` column.
        from_layer (str): The layer to take the data from.
        to_layer (str): The layer to store the results of the transformation.
...
def add_noisy_layer(adata: ad.AnnData, prediction_layer: str) -> ad.AnnData:
    ...
    The originally missing values must be specified by the binary ``adata.layers['mask']`` layer. The name of the added layer will be ``f'noisy_{prediction_layer}'``
    The gene means used to compute the deltas must be stored in the ``adata.var[f'{prediction_layer.replace("deltas", "log1p")}_avg_exp']`` column.
    ...
    Returns:
        ad.AnnData: The updated AnnData object with the noisy layer added in ``adata.layers[f'noisy_{prediction_layer}']``.
    """
    
    if 'deltas' in prediction_layer:
        # If there are deltas in the prediction key then the gene mean should be from the log1p correspondent layer. 
        mean_key = f'{prediction_layer.replace("deltas", "log1p")}_avg_exp'
        # Get vector with gene means
        gene_means = adata.var[mean_key].values
...
# TODO: Add reference to TPM normalization paper
# FIXME: In this pipeline we should include a denoising with spackle
# TODO: Add SEPAL reference to point 3. on denoising
# TODO: Add moran I reference
# TODO: Put combat reference
# TODO: Think in changing the d_ prefix to m_ for median filter
def process_dataset(adata: ad.AnnData, param_dict: dict) -> ad.AnnData:
    ...
    and creates multiple layers. It doesn't perform any filtering or spots modifications. The processing pipeline is the following:

        1. Normalize the data with TPM normalization (``adata.layers['tpm']``)
        2. Logarithmically transform the data following :math:`\log_2(TPM+1)` (``adata.layers['log1p']``)
        ...
        7. Compute the deltas from the mean for each gene in various layers (gets ``'deltas'``, ``'d_deltas'`` and ``'c_deltas'``, ``'c_d_deltas'`` layers)
        8. Add a binary mask layer specifying valid (not imputed) observations for metric computation (``adata.layers['mask']`` layer, ``True`` for valid observations, ``False`` for missing values).

    Args:
        adata (ad.AnnData): The AnnData object to process. Should be already filtered with the ``filter_dataset()`` function or have an analog processing.
